...
abstract class OpenAINetworkingClient {
  ...
  static Future<File> postAndExpectFileResponse({
    ...
    final fileExtensionFromBodyResponseFormat =
        outputFileExtension ?? response.headers[fileTypeHeader]?.split("/").last ?? "mp3";
  ...
  static Future<http.Response> postAndGetResponse({
    ...
    final response = client == null
        ? await http.post(uri, headers: headers, body: handledBody).timeout(OpenAIConfig.requestsTimeOut)
  ...
  static Future<T> fileUpload<T>({
    ...
    assert(file != null || fileBytes != null, "Either file or fileBytes must be provided");
    ...
    final multiPartFile = await () async {
      final MediaType? mime = await mediaTypeFromFile(fileName!);
      if (file != null) {
  ...
  static Future<MediaType?> mediaTypeFromFile(String name) async {
    try {
      final String? mimeType = lookupMimeType(name);
      if (mimeType == null) {
        return null;
      }
